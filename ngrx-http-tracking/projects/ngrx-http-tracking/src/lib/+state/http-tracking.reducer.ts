...
export const httpTrackingAdapter: EntityAdapter<HttpTrackingEntity> = createEntityAdapter<HttpTrackingEntity>({
    selectId: x => x.action,
});
...
export const httpTrackingReducer = createReducer(
    initialState,
    on(HttpTrackingActions.trackHttpRequest, (state, { action, httpStatus, tags }) =>
    ...
    on(HttpTrackingActions.clearGloballyHandledErrors, state => {
        const actionsToUpdate = (<string[]>state.ids)
            ...
            .filter(
                httpTracking =>
                    !!httpTracking && (<Error>httpTracking?.httpStatus)?.message && httpTracking.tags.includes('global')
            )
            .map(tracking => <HttpTrackingEntity>{ ...tracking, httpStatus: LoadingState.INIT });
