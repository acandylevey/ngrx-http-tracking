...
import { isError } from '@campion/utils';
...
export class HttpTrackingFacade {
    ...
    public getTracking(action: Action): Observable<HttpTrackingEntity | undefined> {
    ...
    public getResolved<T1, T2>(action: TrackingAction<T1, T2>): Observable<HttpTrackingResult<T1, T2>> {
        return this.getTracking(action.loading).pipe(
            ...
            take(1),
            map(httpStatus => {
                const retVal = <HttpTrackingResult<T1, T2>>{ action, success: httpStatus === LoadingState.LOADED };
